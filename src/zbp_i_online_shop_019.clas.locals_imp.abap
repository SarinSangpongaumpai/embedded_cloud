CLASS lsc_zbp_i_online_shop_019 DEFINITION INHERITING FROM cl_abap_behavior_saver.
  PROTECTED SECTION.
    METHODS save_modified REDEFINITION.
ENDCLASS.

CLASS lsc_zbp_i_online_shop_019 IMPLEMENTATION.

  METHOD save_modified.

    DATA : lt_online_shop_as TYPE STANDARD TABLE OF zshop_as_019,
    ...
    IF create-online_shop IS NOT INITIAL.
     ...
     " lt_online_shop_as[ 1 ]-purchasereqn =  ls_pr_key-purchaserequisition .
       lt_online_shop_as[ 1 ]-purchasereqn =  '200' .
      INSERT zshop_as_019 FROM TABLE @lt_online_shop_as.
    ENDIF.
  ENDMETHOD.
ENDCLASS.

CLASS lhc_zbp_i_online_shop_019  DEFINITION INHERITING FROM cl_abap_behavior_handler.
  PRIVATE SECTION.
...
CLASS lhc_zbp_i_online_shop_019  IMPLEMENTATION.
...
TRY.
      ...
      WITH VALUE #( ( %cid_ref = 'My%ItemCID_1'
                      %target  = VALUE #( (
                                          ...
                                          textobjecttype = 'B01'
                                          language       = 'E'
                                          plainlongtext  = 'item text created from PAAS API 019 - Workshop prep.'
                                        ) (
                                          %cid           = 'My%CCT_2'
                                          textobjecttype = 'B02'
                                          language       = 'E'
                                          plainlongtext  = 'item2 text created from PAAS API 019'
                                        ) )
              )   )
      ...
      zbp_i_online_shop_019=>cv_pr_mapped = ls_pr_mapped.
...
DATA:
      online_shops TYPE TABLE FOR UPDATE zi_online_shop_019,
      online_shop  TYPE STRUCTURE FOR UPDATE zi_online_shop_019.

    READ ENTITIES OF zi_online_shop_019 IN LOCAL MODE
       ENTITY Online_Shop
        ALL FIELDS
    ...
    LOOP AT lt_online_shop_result INTO DATA(online_shop_read).
      ...
      ENDIF.
    ENDLOOP.
    MODIFY ENTITIES OF zi_online_shop_019 IN LOCAL MODE
    ENTITY zi_online_shop_019 UPDATE SET FIELDS WITH online_shops
    MAPPED   DATA(ls_mapped_modify)
    FAILED   DATA(lt_failed_modify)
    REPORTED DATA(lt_reported_modify).

    DATA: lt_create_pr_imp TYPE TABLE FOR ACTION IMPORT  zi_online_shop_019~create_pr,
